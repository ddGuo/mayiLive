...
from services.douyin.dy import PushFrame
from services.douyin.dy import Response
from services.douyin.dy import MatchAgainstScoreMessage
from services.douyin.dy import LikeMessage
from services.douyin.dy import MemberMessage
from services.douyin.dy import GiftMessage
from services.douyin.dy import ChatMessage
from services.douyin.dy import SocialMessage
from services.douyin.dy import RoomUserSeqMessage
from services.douyin.dy import UpdateFanTicketMessage
from services.douyin.dy import CommonTextMessage
from services.douyin.dy import ProductChangeMessage
...
def unPackWebcastChatMessage(data):
    ...
    log = json.dumps(data, ensure_ascii=False)
    # logger.info(f'[unPackWebcastChatMessage] [直播间弹幕消息{GlobalVal.commit_num}] [房间Id：' + liveRoomId + '] | ' + log)
    # logger.info(f'[unPackWebcastChatMessage] [直播间弹幕消息{GlobalVal.commit_num}] [房间Id：' + liveRoomId + '] | ' + data['content'])
    print(data)
    ws_app.ws_send(f"{data['content']}")
    return data
...
def wssServerStop(dict):
    if global_ws and dict.get("ws_danmu") == "stop":
        global_ws.close()
...
""".strip()
        js_enc = load_webmssdk('./douyin/webmssdk.js')
...
def parseLiveRoomUrl(url):
    ...
    headers = {
        ...
        'referer': 'https://live..douyin.dy.com/721566130345?cover_type=&enter_from_merge=web_live&enter_method=web_card&game_name=&is_recommend=&live_type=game&more_detail=&room_id=7317569386624125734&stream_type=vertical&title_type=&web_live_tab=all',
    ...
    if res_room_info:
        ...
        print(f"房间标题: {liveRoomTitle}")
        if room_status == '4':
            base_model.emit_signal({"msg_code" : "1"})
    ...
    # wss_url = "wss://webcast5-ws-web-hl..douyin.dy.com/webcast/im/push/v2/?app_name=douyin_web&version_code=180800&webcast_sdk_version=1.0.14-beta.0&update_version_code=1.0.14-beta.0&compress=gzip&device_platform=web&cookie_enabled=true&screen_width=1920&screen_height=1080&browser_language=zh-CN&browser_platform=Win32&browser_name=Mozilla&browser_version=5.0%20(Windows%20NT%2010.0;%20Win64;%20x64)%20AppleWebKit/537.36%20(KHTML,%20like%20Gecko)%20Chrome/131.0.0.0%20Safari/537.36&browser_online=true&tz_name=Asia/Shanghai&cursor=r-1_d-1_u-1_h-1_t-1732688916245&internal_ext=internal_src:dim|wss_push_room_id:7441600848641215267|wss_push_did:7382771879608337935|first_req_ms:1732688916168|fetch_time:1732688916245|seq:1|wss_info:0-1732688916245-0-0|wrds_v:7441842202591890343&host=https://live..douyin.dy.com&aid=6383&live_id=1&did_rule=3&endpoint=live_pc&support_wrds=1&user_unique_id=7382771879608337935&im_path=/webcast/im/fetch/&identity=audience&need_persist_msg_count=15&insert_task_id=&live_reason=&room_id=7441600848641215267&heartbeatDuration=0&signature=fgyCTiUdP84+QSdH"
    # wss_url = "wss://webcast5-ws-web-lq..douyin.dy.com/webcast/im/push/v2/?app_name=douyin_web&version_code=180800&webcast_sdk_version=1.0.14-beta.0&update_version_code=1.0.14-beta.0&compress=gzip&device_platform=web&cookie_enabled=true&screen_width=1920&screen_height=1080&browser_language=zh-CN&browser_platform=Win32&browser_name=Mozilla&browser_version=5.0%20(Windows%20NT%2010.0;%20Win64;%20x64)%20AppleWebKit/537.36%20(KHTML,%20like%20Gecko)%20Chrome/131.0.0.0%20Safari/537.36&browser_online=true&tz_name=Asia/Shanghai&cursor=u-1_h-7441843984294499379_t-1732689333561_r-1_d-1&internal_ext=internal_src:dim|wss_push_room_id:7441835240294894377|wss_push_did:7382771879608337935|first_req_ms:1732689333470|fetch_time:1732689333561|seq:1|wss_info:0-1732689333561-0-0|wrds_v:7441844015068087878&host=https://live..douyin.dy.com&aid=6383&live_id=1&did_rule=3&endpoint=live_pc&support_wrds=1&user_unique_id=7382771879608337935&im_path=/webcast/im/fetch/&identity=audience&need_persist_msg_count=15&insert_task_id=&live_reason=&room_id=7441835240294894377&heartbeatDuration=0&signature=6BZs4FurkUgEBof4"
    # wss_url = "wss://zj-cn-live-comet.chat.bilibili.com:2245/sub"
    # wss_url = "wss://webcast5-ws-web-lf.douyin.com/webcast/im/push/v2/?app_name=douyin_web&version_code=180800&webcast_sdk_version=1.0.14-beta.0&update_version_code=1.0.14-beta.0&compress=gzip&device_platform=web&cookie_enabled=true&screen_width=1920&screen_height=1080&browser_language=zh-CN&browser_platform=Win32&browser_name=Mozilla&browser_version=5.0%20(Windows%20NT%2010.0;%20Win64;%20x64)%20AppleWebKit/537.36%20(KHTML,%20like%20Gecko)%20Chrome/131.0.0.0%20Safari/537.36&browser_online=true&tz_name=Asia/Shanghai&cursor=t-1732781476063_r-1_d-1_u-1_h-1&internal_ext=internal_src:dim|wss_push_room_id:7442016000540773171|wss_push_did:7382771879608337935|first_req_ms:1732781476012|fetch_time:1732781476063|seq:1|wss_info:0-1732781476063-0-0|wrds_v:7442239753354747799&host=https://live.douyin.com&aid=6383&live_id=1&did_rule=3&endpoint=live_pc&support_wrds=1&user_unique_id=7382771879608337935&im_path=/webcast/im/fetch/&identity=audience&need_persist_msg_count=15&insert_task_id=&live_reason=&room_id=7442016000540773171&heartbeatDuration=0&signature=fMjuR88uU2ZYbJDB"
    # wss_url = "wss://webcast5-ws-web-lf.douyin.com/webcast/im/push/v2/?app_name=douyin_web&version_code=180800&webcast_sdk_version=1.0.14-beta.0&update_version_code=1.0.14-beta.0&compress=gzip&device_platform=web&cookie_enabled=true&screen_width=1920&screen_height=1080&browser_language=zh-CN&browser_platform=Win32&browser_name=Mozilla&browser_version=5.0%20(Windows%20NT%2010.0;%20Win64;%20x64)%20AppleWebKit/537.36%20(KHTML,%20like%20Gecko)%20Chrome/131.0.0.0%20Safari/537.36&browser_online=true&tz_name=Asia/Shanghai&cursor=t-1732782770931_r-1_d-1_u-1_h-1&internal_ext=internal_src:dim|wss_push_room_id:7442016000540773171|wss_push_did:7382771879608337935|first_req_ms:1732782770889|fetch_time:1732782770931|seq:1|wss_info:0-1732782770931-0-0|wrds_v:7442245323927327095&host=https://live.douyin.com&aid=6383&live_id=1&did_rule=3&endpoint=live_pc&support_wrds=1&user_unique_id=7382771879608337935&im_path=/webcast/im/fetch/&identity=audience&need_persist_msg_count=15&insert_task_id=&live_reason=&room_id=7442016000540773171&heartbeatDuration=0&signature=f/WVde2nPeoi1JRl"
    base_model.emit_signal({"msg_code" : "2", "room_nickname":live_room_info["nickname"]})
